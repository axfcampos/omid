...
public class Transaction implements Comparable<Transaction> {
    ...
    private long commit_ts; //might be usefull to recollect serial order
    ...
    public void update(Transaction tx, RowKey[] rows){
        ...
        for (Operation op : this.ops.keySet()){
            for ( RowKey row : rows){
                if(op.getHash() == row.hashCode()){
                    op.updateKValues(row.getValue());
